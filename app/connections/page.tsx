      ...
      <div className="space-y-6">
              ...
              <tbody>
                {connections.map(conn => (
                  <tr key={conn.id} className="border-b border-white/10 hover:bg-white/5 transition">
                    ...
                    <td className="py-3 px-4 text-slate-300">{conn.db_type === 'postgresql' ? 'PostgreSQL' : 'MSSQL'}</td>
                    <td className="py-3 px-4 text-slate-300">{conn.host}</td>
                    <td className="py-3 px-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        conn.status === 'active' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'
                      }`}>
                        {conn.status === 'active' ? 'Active' : 'Paused / Disconnected'}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-slate-400 text-xs">{conn.last_checked_at ? new Date(conn.last_checked_at).toLocaleString() : 'N/A'}</td>
                    <td className="py-3 px-4">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-[#0c1628] border border-white/10 rounded-[1.5rem] p-6">
            ...
            <p className="text-slate-400 text-sm mb-4">DB Autopilot has detected new nodes in your infrastructure. Connect them to start monitoring query execution plans across clusters.</p>
          ...
          <div className="bg-[#0c1628] border border-white/10 rounded-[1.5rem] p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-400 font-semibold">{connections.filter(conn => conn.status === 'active').length} / {connections.length}</span>
              </div>
              <div className="w-full bg-[#0a0f1a] rounded-full h-2">
                <div className="bg-green-500 h-2 rounded-full" style={{ width: `${connections.length ? (connections.filter(conn => conn.status === 'active').length / connections.length) * 100 : 0}%` }}></div>
              </div>
            </div>
          </div>
        </div>
