...
export async function GET(req: NextRequest) {
  try {
    ...
    const connections = await query(
      `SELECT id, name, host, port, db_name, username, db_type,
              status, last_checked_at, last_error, created_at
...
export async function POST(req: NextRequest) {
  try {
    ...
    const newConn = await queryOne<any>(
      `INSERT INTO monitored_connections
         (name, host, port, db_name, username, password_encrypted, db_type, added_by, status, last_checked_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9, NOW())
       RETURNING id, name, host, port, db_name, username, db_type, status, created_at`,
      [
        name, host, port, db_name, username,
        encrypt(password),
        db_type,
        authUser.userId,
        test.success ? 'active' : 'error',
      ]
    )
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await queryOne<any>(
      `UPDATE monitored_connections SET status = $1, updated_at = NOW()
       WHERE id = $2
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const conn = await queryOne<any>(
      'SELECT id, db_type FROM monitored_connections WHERE id = $1', [id]
    )
